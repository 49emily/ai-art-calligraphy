...
class BaseOptions:
    ...
    def initialize(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="/tmp/output",
            help="directory to store output in Modal environment",
        )
        parser.add_argument(
            "--modal_data_mount",
            action="store_true",
            help="if specified, data is mounted from a shared volume in Modal",
        )
    ...
    def print_options(self, opt):
        ...
        try:
            ...
            print(f"Warning: Failed to save options file to {file_name}: {e}")
    ...
    def parse(self):
        ...
        if len(opt.gpu_ids) > 0:
            try:
                ...
                print(f"Warning: CUDA error when setting device: {e}")
                print("Will continue with available devices")
                # Try to use available devices if any
        ...
        # For Modal environments, adjust paths if needed
        if getattr(opt, "modal_data_mount", False):
            # If data is mounted in Modal, ensure the paths are correct
            if not os.path.isabs(opt.dataroot):
                opt.dataroot = os.path.join("/code", opt.dataroot)

            # Ensure checkpoints directory exists in the mounted volume
            if not os.path.isabs(opt.checkpoints_dir):
                opt.checkpoints_dir = os.path.join(opt.modal_output_dir, "checkpoints")
