...
# Check for GPU availability (either CUDA or MPS for Mac)
if torch.cuda.is_available():
    ...
    logging.warning(
        "Neither CUDA nor MPS is available. Using CPU which will be significantly slower."
    )
...
def load_referance(reference_root, index_path="./assets/reference/index.pkl"):
    ...
    for writer in range(len(index)):
        for character in range(len(index[writer])):
            index[writer][character] = os.path.join(
                reference_root, index[writer][character]
            )
    return index
...
class MetaScript:
    def __init__(self, ref_path, model_path) -> None:
        ...
        self.generator_model = SynthesisGenerator(reference_count=4).to(device)
        ...
        self.punctuation_remap = {
            value: key for key, value in self.punctuation_map.items()
        }
    ...
    def process_reference(self, image_list, path):
        ...
        reference_image = [
            np.array(self.align_transform(image)) for image in image_list
        ]
        ...
        logging.info(
            "Reference image saved to {}".format(os.path.join(path, "reference.png"))
        )
    ...
    def generate(self, target_text, reference, size, width, path):
        ...
        for idx, word in tqdm(
            enumerate(target_text), desc="generating script"
        ):  # to make video
            if word == " ":
                ...
                image = Image.open(
                    os.path.join(
                        "./assets/character",
                        "{}.png".format(self.character_remap[word]),
                    )
                )
                template = self.input_transform(image).unsqueeze(0).to(device)
                with torch.no_grad():
                ...
                image = Image.open(
                    os.path.join(
                        "./assets/punctuation",
                        "{}.png".format(self.punctuation_remap[word]),
                    )
                )
